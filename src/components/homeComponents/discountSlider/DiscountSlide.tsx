...
const DiscountSlide = ({ product }: { product: Product }) => {
  ...
  return (
    ...
    >
      <span className="absolute pl-24 w-56 h-8  text-white -right-8 -top-5 rotate-[37deg] flex justify-center items-center gap-1">
        <SaleIcon />
        Sale
      </span>
      <div className="w-full shrink-0 h-[70%]   overflow-hidden mx-auto">
        <img
      ...
      <div className="flex justify-between items-center mt-auto">
        <div className="flex gap-2">
          ...
          <p className="text-lg sm:text-base text-neutral-300 relative">
            {+product?.price + 1000}$
            <span className="absolute top-3 sm:top-[0.55rem] left-0 w-12 h-[0.1rem] "></span>
          </p>
        </div>
